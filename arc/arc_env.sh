...
cd "$REPO_ROOT"
[[ -f training/train.py && -d configs/mazes ]] \
...
# Default: REUSE the payload-transport project's `rtn` env. It is the identical,
# already-verified stack (Isaac Sim 4.5.0 + Isaac Lab v2.1.0 + rsl_rl 2.3.1) and a
# second copy would cost ~35 GB of quota. The playbook's "never reuse envs" rule is
# about envs with DIFFERENT pins (vLLM/TRL); set DRONE_ENV to split them if pins diverge.
: "${DRONE_ENV:=$HOME/miniconda3/envs/rtn}"
[[ "$DRONE_ENV" = /* ]] || _die "DRONE_ENV must be an absolute path, got '$DRONE_ENV'"
PY="$DRONE_ENV/bin/python"
[[ -x "$PY" ]] || _die "no python at $PY -- run arc/setup_env.sh first"
export PATH="$DRONE_ENV/bin:$PATH"
...
"$PY" -c "import sys; assert sys.prefix.startswith('$DRONE_ENV'), sys.prefix" \
  || _die "python is not running inside $DRONE_ENV"
...
sys.exit('missing: ' + ', '.join(missing) if missing else 0)" \
  || _die "Isaac stack incomplete in $DRONE_ENV -- re-run arc/setup_env.sh"
...
_secrets="${DRONE_SECRETS:-$HOME/.config/vrr/secrets.env}"
...
${tail_txt}"
  ...
  arc_notify "[drone] ${SLURM_JOB_NAME:-job} ${SLURM_JOB_ID:-} exit=$rc on $(hostname)
${tail_txt}"
  return $rc
}
