...
# =============================================================================
# THIS PROJECT: by default DRONE_ENV points at the payload-transport project's
# existing `rtn` env (same pins). If it already exists, run only:
#     bash arc/setup_env.sh --verify
# which also installs nothing -- it checks versions, pip conflicts and runs tests/.
# Missing project extras (imageio[ffmpeg], matplotlib) -> `$DRONE_ENV/bin/pip install -r requirements.txt`.
set -euo pipefail

DRONE_ENV="${DRONE_ENV:-$HOME/miniconda3/envs/rtn}"
...
preflight() {
  ...
  if [[ -t 0 ]]; then
    ...
    [[ "$ok" == [yY]* ]] || die "free up quota first (or set DRONE_ENV to a location with room)"
  fi
...
install() {
  ...
  export TMPDIR="$HOME/tmp/drone-pip"
  mkdir -p "$TMPDIR"
  local PIP=("$PY" -m pip install --no-cache-dir)

  if [[ ! -x "$PY" ]]; then
    say "creating env $DRONE_ENV (python $PY_VERSION)"
    "$CONDA_BIN" create -y -p "$DRONE_ENV" "python=$PY_VERSION"
  else
    say "env exists: $DRONE_ENV"
  fi
  ...
  (
    export CONDA_PREFIX="$DRONE_ENV" PATH="$DRONE_ENV/bin:$PATH"
...
EOF
  ...
  say "VERIFY OK. Next: arc/validate.slurm -- see README (Running on ARC)."
}
